...
*/
static unsigned char rev_one_byte(unsigned char byte)
{
...
static unsigned int rev_four_bytes(unsigned int bytes)
{
    unsigned int reversed = 0;

    for (my_size_t i = 0; i < 32; i++) {
        reversed <<= 1;
        reversed |= (bytes & 1);
        bytes >>= 1;
    }
...
static unsigned long int rev_eight_bytes(unsigned long int bytes)
{
    unsigned int reversed = 0;
...
{
    unsigned char tmp_one = 0;
    unsigned int tmp_four = 0;
    unsigned long int tmp_eight = 0;

    if (bytes == 1) {
        tmp_one = rev_one_byte(*(unsigned char *)mem);
        return my_memcpy(mem, &tmp_one, 1);
    }
    if (bytes == 4) {
        tmp_four = rev_four_bytes(*(unsigned int *)mem);
        return my_memcpy(mem, &tmp_four, 4);
    }
    if (bytes == 8) {
        tmp_eight = rev_eight_bytes(*(unsigned long int *)mem);
